class PlantphotosController < ApplicationController
  ...
  def create
    ...
    if the_plantphoto.valid?
      the_plantphoto.save
      redirect_to("/plantphotos", { :notice => "Plantphoto created successfully." })
    else
      redirect_to("/plantphotos", { :alert => the_plantphoto.errors.full_messages.to_sentence })
    end
  end
  ...
  def destroy
    ...
    redirect_to("/plantphotos", { :notice => "Plantphoto deleted successfully."} )
  end
end
